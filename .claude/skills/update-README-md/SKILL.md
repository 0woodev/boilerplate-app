...
description: 프로젝트 소스코드 전반을 읽어서 app 레포 + 등록된 모든 서브모듈(be, fe 등)의 README.md를 최신 상태로 업데이트합니다. 프로젝트명, 스택, 구조, 사용법, 배포 정보 등을 코드 기반으로 생성.
---

프로젝트 전체의 README.md를 소스코드 기반으로 최신화한다.

**대상:** 루트 레포 + `.gitmodules` 에 등록된 모든 서브모듈 (be/, fe/ 기본 + 존재하는 서브모듈 전부)
...
- `PROGRESS.md` — 현재 진행 상태, 완료/진행중/미착수 항목 (있는 경우)
- `DOMAIN.md` — 도메인 설계 (있는 경우)
- `.gitmodules` — 등록된 서브모듈 목록
- `dev.env` 또는 `sample.env` — PROJECT_NAME, DOMAIN 등
...
`.gitmodules` 에 등록된 서브모듈마다 다음을 확인:
...
**기타 서브모듈:**
- `.gitmodules` 에 추가로 등록된 서브모듈이 있으면 해당 모듈의 README.md, CLAUDE.md, 소스 구조를 읽어서 동일하게 분석
